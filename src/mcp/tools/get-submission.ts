...
/**
 * get_submission MCP tool implementation.
 * Downloads submission content (PDF/images), converts to optimized images,
 * and returns as MCP multimodal content blocks for Claude visual analysis.
 */
...
import { formatAsMultimodalContent } from "../../content/multimodal-formatter.js";

const MAX_BINARY_SIZE = 50 * 1024 * 1024; // 50MB

/**
 * Register the get_submission tool with the MCP server.
 */
export function registerGetSubmissionTool(
  server: McpServer,
  ...
  server.registerTool(
    "get_submission",
    {
      title: "Get Submission Content",
      description:
        "Download submission PDF/images for a graded assignment and return them as images for visual analysis. " +
        "Supports optional question targeting to retrieve only relevant pages. " +
        "Returns base64-encoded page images as multimodal content blocks.",
      inputSchema: z.object({
        ...
        question: z
          ...
          .optional()
          .describe(
            "Optional question to target (e.g., 'Q3', 'Question 1'). Returns only pages for that question. Omit for full submission."
          ),
        forceRefresh: z
          .boolean()
          .optional()
    ...
    async ({ course, assignment, question, forceRefresh }) => {
      try {
        ...
        // 6. Parse submission page for download URL and metadata
        const submissionContent = parseSubmissionPage(html);

        // 7. Determine target pages based on question (if provided)
        const totalPages = submissionContent.totalPages ?? 1;
        const targetPages = determineTargetPages(
          totalPages,
          question,
          submissionContent.pageMappings
        );

        // 8. Download and convert content based on content type
        ...
        if (submissionContent.contentType === "pdf") {
        ...
        } else if (submissionContent.pageImageUrls && submissionContent.pageImageUrls.length > 0) {
          ...
          log("DEBUG", `Downloading single submission image from ${submissionContent.downloadUrl}`);
          const response = await gsClient.getRaw(submissionContent.downloadUrl);
          const imageBuffer = Buffer.from(await response.arrayBuffer());
          const optimized = await optimizeForClaudeVision(imageBuffer);
        ...
        // 9. Format as multimodal content blocks
        const contentBlocks = formatAsMultimodalContent({
